...
console.log(`${task}1`);
const arr = ['a', 'b', 'c', 'd'];
const str1 = arr.slice(0, 2)
  .join(' + ');
const str2 = arr.slice(2, 4)
  .join(' + ');
console.log(`${str1} \n${str2}`);
...
console.log(`Кількість елементів, які потрібно додати з початку масиву, щоб отримати суму більше 10: ${count}`);

//task #9
console.log(`${task}9`);
const numbers = [1, 2, 3, 4, 5, 7, 9, 11];
for (let i = 0; i < numbers.length / 2; i++) {
  let swap = numbers[numbers.length - 1 - i];
...
for (let a = 0; a < arr10.length; a++) {
  for (let b = 0; b < arr10[a].length; b++) {
    sum2 += arr10[a][b];
  }
}
console.log(sum2);
...
let sum3 = 0;
for (let a = 0; a < arr11.length; a++) {
  for (let b = 0; b < arr11[a].length; b++) {
    for (let c = 0; c < arr11[b].length; c++) {
      sum3 += arr11[a][b][c];
    }
  }
}
